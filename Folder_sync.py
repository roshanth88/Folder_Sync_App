...
def folder_sync_main():
    """
    Parse the command line arguments and start the folder synchronization.
    """
    ...
    sync_logger = SingletonLogger(args.log_file_path)

    check_for_valid_dir(args.source_folder, "Source")
    check_for_valid_dir(args.replica_folder, "Replica")

    sync_on = True
    sync_logger.logger.info(f"Source synchronization folder {args.source_folder}")
    sync_logger.logger.info(f"Replica Synchronization folder {args.replica_folder}")
    sync_logger.logger.info(f"Log file location {args.log_file_path}")
    sync_logger.logger.info(f"Synchronization interval {args.sync_interval}")
